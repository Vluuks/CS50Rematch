...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // create array to store words in request line
            char* word[3];

            // divide line into 3 words based on spaces and store them in array
            ...
            // prepare room for strings for strcopy
            char method[100];
            char target[100];
            char version[100];
            
            // take strings from array and give them more descriptive names 
            if (word[0] != NULL && has_space(word[0]) == false)
                strcpy(method, word[0]);
            
            if (word[1] != NULL && has_space(word[1]) == false)
                strcpy(target, word[1]);
            
            if (word[2] != NULL && has_space(word[2]) == false)
                strcpy(version, word[2]);

            // compare strings against their further requirements
            // if method is not GET, return error 405
            ...
            // if the first character of request-target is not / return error 501
            if (target[0] != '/')
            {
                error(501);
                continue;
            ...
            // if the request-target contains a ", return error 400
            ...
            // check is http version is correct
            ...
            for (int i = 0; i < strlen(target); i++)
            {
                ...
                // if start of query is encountered, break out of loop
                else if (target[i] == '?')
                break; 
            }
            
            // if no dots were found in part before query, give error
            if (dot == 0)
            {
                error(501);
                continue;
            ...
            // if a query is present extract path and query
            if (strrchr(target, '?') != NULL)
            {
                // create array to store words of path and query
                char* word_ii[2];

                // divide line into 3 words and store them in array
                char* token_ii = strtok(target, "?");
                int k = 0;
                ...
                while (token_ii != NULL)
                {
                    word_ii[k] = token_ii;
                    k++;
                    token_ii = strtok(NULL, "?");
                }
            
                // extract query and path from request-target
                char pathnoroot[500];
                strcpy(query, word_ii[1]); 
                strcpy(pathnoroot, word_ii[0]);
                
                // copy the root so strcat doesnt alter it
                char root_save_q[500]; 
                strcpy(root_save_q, root);
                
                // add root and path together for complete path
                strcpy(path, strcat(root_save_q, pathnoroot));
                

            }
            
            // if no query is found, treat the full target-line as path and append to root
            else if (strrchr(target, '?') == NULL)
            { 
                  // copy the root so strcat doesnt alter it
                  char root_save[500];  
                  strcpy(root_save, root);
                  
                  // append target to root  
                  strcpy(path,strcat(root_save,target));
            ...
            char buffer[5];
            sprintf(buffer,"%s", ext_start + 1);
                       
            // store extension in variable
            char extension[5];
...
/**
 * Checks whether a word contains spaces
 */
bool has_space(char* word)
{
    for(int i = 0; i < strlen(word); i++)
    {
        if (isblank(word[i]) == true)
            return true;
    }
...
const char* lookup(const char* extension)
{
    // compare imput against possible extensions
